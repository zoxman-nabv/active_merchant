  ...
  module Billing #:nodoc:
    ...
    class BogusGateway < Gateway
      ...
      def authorize_emv(money, paysource, options = {})
        ...
        case money
        when /00$/
          Response.new(true, SUCCESS_MESSAGE, {:authorized_amount => money}, :test => true, :authorization => AUTHORIZATION, :emv_authorization => "8A023030" )
        when /05$/
          Response.new(false, FAILURE_MESSAGE, {:authorized_amount => money, :error => FAILURE_MESSAGE }, :test => true, :error_code => STANDARD_ERROR_CODE[:processing_error], :emv_authorization => "8A023035" )
        else
          raise Error, error_message(paysource)
        end
      end
      ...
      def purchase_emv(money, paysource, options = {})
        ...
        case money
        when /00$/
          Response.new(true, SUCCESS_MESSAGE, {:paid_amount => money}, :test => true, :authorization => AUTHORIZATION, :emv_authorization => "8A023030")
        when /05$/
          Response.new(false, FAILURE_MESSAGE, {:paid_amount => money, :error => FAILURE_MESSAGE }, :test => true, :error_code => STANDARD_ERROR_CODE[:processing_error], :emv_authorization => "8A023035")
        else
          raise Error, error_message(paysource)
        end
      end
